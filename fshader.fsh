...
void main(void)
{
    ...
    vec4 diffColor = diffMatColor * u_lightPower * max(0.0, dot(v_normal, -lightVect));// / (1.0 + 0.25 * pow(len, 2.0f)); добаляет затухание света при иудалении
    ...
    gl_FragColor = resultColor; //texture2D(u_texture, v_texcoord);//vec4(1.0, 0.0, 0.0, 1.0);//texture2D(qt_Texture0, qt_TexCoord0);
}
